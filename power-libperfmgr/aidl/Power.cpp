...
ndk::ScopedAStatus Power::setMode(Mode type, bool enabled) {
    CALL_HOOK(setMode, type, enabled)
...
ndk::ScopedAStatus Power::isModeSupported(Mode type, bool *_aidl_return) {
    CALL_HOOK(isModeSupported, type, _aidl_return)
...
ndk::ScopedAStatus Power::setBoost(Boost type, int32_t durationMs) {
    CALL_HOOK(setBoost, type, durationMs)
...
ndk::ScopedAStatus Power::isBoostSupported(Boost type, bool *_aidl_return) {
    CALL_HOOK(isBoostSupported, type, _aidl_return)
    bool supported = HintManager::GetInstance()->IsHintSupported(toString(type));
...
ndk::ScopedAStatus Power::createHintSession(int32_t tgid, int32_t uid,
    ...
    CALL_HOOK(createHintSession, tgid, uid, threadIds, durationNanos, _aidl_return)
    if (!HintManager::GetInstance()->GetAdpfProfile() ||
        HintManager::GetInstance()->GetAdpfProfile()->mReportingRateLimitNs <= 0) {
...
ndk::ScopedAStatus Power::getHintSessionPreferredRate(int64_t *outNanoseconds) {
    CALL_HOOK(getHintSessionPreferredRate, outNanoseconds)
