...
int main() {
    ...
    if (android::base::GetBoolProperty(kConfigDebugPathProperty.data(), false)) {
        ...
        LOG(WARNING) << "Lineage Power HAL AIDL Service is using debug config from: " << config_path;
    }
    config_path.append(
            android::base::GetProperty(kConfigProperty.data(), kConfigDefaultFileName.data()));

    LOG(INFO) << "Lineage Power HAL AIDL Service with Extension is starting with config: "
              << config_path;
