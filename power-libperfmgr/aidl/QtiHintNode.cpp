...
void QtiHintNode::DumpToFd(int fd) const {
    ...
    if (inactive_hint_id_.has_value()) {
        buf = android::base::StringPrintf(
                "%s\tstate=%s\tactive_hint_id=0x%x\tinactive_hint_id=0x%x\ttype=%d\n",
                name_.c_str(), state, hint_id_, *inactive_hint_id_, hint_type_);
    } else {
        buf = android::base::StringPrintf("%s\tstate=%s\thandle=%d\thint_id=0x%x\ttype=%d\n",
...
bool QtiHintNode::ApplyActiveHint(std::size_t value_index, bool log_error) {
    ...
    if (binding_.perfHintAcqRel != nullptr) {
        const int old_handle = CheckHandle(handle_) ? handle_ : kInvalidHandle;
        ret = binding_.perfHintAcqRel(old_handle, hint_id_, kPerfPackage, hal_duration_ms,
                                      hint_type_, 0, nullptr);
        LOG(VERBOSE) << "binding_.perfHintAcqRel node=" << name_ << " handle=" << old_handle
                     << " hint_id=0x" << std::hex << hint_id_ << std::dec << " type=" << hint_type_
                     << " duration_ms=" << hal_duration_ms << " ret=" << ret;
    } else {
        if (CheckHandle(handle_)) {
            const int release_ret = binding_.releasePerfLock(handle_);
            LOG(VERBOSE) << "binding_.releasePerfLock node=" << name_ << " handle=" << handle_
                         << " ret=" << release_ret;
            if (release_ret < 0 && log_error) {
    ...
    if (!CheckHandle(ret)) {
        if (binding_.perfHintAcqRel != nullptr && log_error) {
            LOG(ERROR) << "Failed to issue QTI perf action id=0x" << std::hex << hint_id_
                       << std::dec << " type=" << hint_type_ << " duration_ms=" << hal_duration_ms;
        }
...
int QtiHintNode::CallPerfHint(int hint_id, int hal_duration_ms, bool log_error) {
    ...
    LOG(VERBOSE) << "binding_.perfHint node=" << name_ << " hint_id=0x" << std::hex << hint_id
                 << std::dec << " type=" << hint_type_ << " duration_ms=" << hal_duration_ms
                 << " ret=" << ret;
    if (!CheckHandle(ret)) {
        if (log_error) {
            LOG(ERROR) << "Failed to issue QTI perf action id=0x" << std::hex << hint_id << std::dec
...
void QtiHintNode::ReleaseHandle(bool log_error) {
    ...
    const int ret = binding_.releasePerfLock(handle_);
    LOG(VERBOSE) << "binding_.releasePerfLock node=" << name_ << " handle=" << handle_
                 << " ret=" << ret;
    if (ret < 0 && log_error) {
        LOG(ERROR) << "Failed to release QTI perf handle for node " << name_
