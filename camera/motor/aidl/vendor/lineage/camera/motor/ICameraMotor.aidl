...
import vendor.lineage.camera.motor.ICameraMotorCallback;
...
interface ICameraMotor {
    ...
    /**
     * Try to quickly close the camera popup to protect the hardware
     * (e.g. due to dropping the device).
     * If the hardware doesn't support this, proxy this call to setState(false).
     */
    void emergencyClose();

    /**
     * Register a callback to be notified when the state of the motor changes.
     * @param callback The callback to register.
     */
    void registerCallback(ICameraMotorCallback callback);

    /**
     * Unregister a callback.
     * @param callback The callback to unregister.
     */
    void unregisterCallback(ICameraMotorCallback callback);
}
