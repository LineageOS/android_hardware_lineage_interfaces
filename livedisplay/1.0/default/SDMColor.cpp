...
DisplayMode SDMColor::invalidDisplayMode() {
    DisplayMode mode;
    mode.id = -1;
    return mode;
}
...
Return<void> SDMColor::native_getCurrentDisplayMode(native_getCurrentDisplayMode_cb _hidl_cb) {
    ...
    if (check(Feature::DISPLAY_MODES)) {
        sp<disp_mode> mode = getCurrentDisplayMode();
        if (mode != nullptr) {
            _hidl_cb(modePointerToObj(mode));
        } else {
            _hidl_cb(invalidDisplayMode());
        }
    }
    return Void();
}
...
Return<void> SDMColor::native_getDefaultDisplayMode(native_getDefaultDisplayMode_cb _hidl_cb) {
    ...
    if (check(Feature::DISPLAY_MODES)) {
        sp<disp_mode> mode = getDefaultDisplayMode();
        if (mode != nullptr) {
            _hidl_cb(modePointerToObj(mode));
        } else {
            _hidl_cb(invalidDisplayMode());
...
Return<void> SDMColor::native_getHueRange(native_getHueRange_cb _hidl_cb) {
    HSICRanges ranges;
    getPictureAdjustmentRanges(ranges);
    _hidl_cb(ranges.hue);
    return Void();
}
