...
#define LOG_TAG "LiveDisplay-QDCM"
    ...
    if (mode->privFlags == PRIV_MODE_FLAG_SYSFS) {
        ...
        if (state) {
            ...
            if (Utils::readInitialModeId(&id) == OK) {
                LOG(VERBOSE) << "set qdcm mode to default: id" << id;
...
status_t QDCM::getDisplayModes(vector<sp<disp_mode>>& profiles) {
    ...
    if (qdcm_count == 0) {
        ...
        for (uint32_t i = 0; i < qdcm_count; i++) delete tmp[i].name;
        delete[] tmp;
    }
...
bool QDCM::hasFeature(Feature feature) {
    ...
    if (QDCMController::getInstance().get_feature_version(mHandle, id, &v, &flags) == 0) {
        if (v.x > 0 || v.y > 0 || v.z > 0) {
            ...
            if (feature == Feature::DISPLAY_MODES || feature == Feature::COLOR_TEMPERATURE) {
...
status_t QDCM::saveInitialDisplayMode() {
    ...
    uint32_t flags = 0;
    if (Utils::readInitialModeId(&id) != OK || id < 0) {
        if (QDCMController::getInstance().get_default_display_mode(mHandle, 0, &id, &flags) == OK &&
            id >= 0) {
