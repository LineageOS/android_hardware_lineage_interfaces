...
status_t QDCM::initialize() {
    status_t rc = NO_INIT;

    rc = QDCMController::getInstance().init(&mHandle, 0);
    if (rc != OK) {
        return rc;
    }
...
status_t QDCM::deinitialize() {
    QDCMController::getInstance().deinit(mHandle, 0);
    mHandle = -1;
    return OK;
}

uint32_t QDCM::getNumQDCMDisplayModes() {
    uint32_t flags = 0;
    int32_t count = 0;
    if (QDCMController::getInstance().get_num_display_modes(mHandle, 0, 0, &count, &flags)) {
        count = 0;
    }
    return count;
}

uint32_t QDCM::getNumDisplayModes() {
    int32_t count = getNumQDCMDisplayModes();
...
sp<disp_mode> QDCM::getDisplayModeById(int32_t id) {
...
status_t QDCM::setModeState(sp<disp_mode> mode, bool state) {
    int32_t id = 0;
    ...
    } else if (mode->privFlags == PRIV_MODE_FLAG_QDCM) {
        if (state) {
            return QDCMController::getInstance().set_active_display_mode(mHandle, 0, mode->id, 0);
        } else {
            if (Utils::readInitialModeId(&id) == OK) {
                ALOGV("set qdcm mode to default: id=%d", id);
                return QDCMController::getInstance().set_active_display_mode(mHandle, 0, id, 0);
            }
        }
    }
    return BAD_VALUE;
...
status_t QDCM::getDisplayModes(vector<sp<disp_mode>>& profiles) {
    status_t rc = OK;

    uint32_t count = getNumDisplayModes();
    if (!count) return rc;

    uint32_t qdcm_count = getNumQDCMDisplayModes();
    ...
    if (qdcm_count == 0) {
        ...
        qdcm_mode* tmp = new qdcm_mode[qdcm_count];
        memset(tmp, 0, sizeof(qdcm_mode) * qdcm_count);
        for (uint32_t i = 0; i < qdcm_count; i++) {
            tmp[i].id = -1;
        ...
        uint32_t flags = 0;
        rc = QDCMController::getInstance().get_display_modes(mHandle, 0, 0, tmp, qdcm_count, &flags);
        if (rc == 0) {
            for (uint32_t i = 0; i < qdcm_count; i++) {
                const sp<disp_mode> m = new disp_mode;
                m->id = tmp[i].id;
                m->name = tmp[i].name;
                m->privFlags = PRIV_MODE_FLAG_QDCM;
                profiles.push_back(m);
            }
        }
        for (uint32_t i = 0; i < qdcm_count; i++)
...
sp<disp_mode> QDCM::getCurrentDisplayMode() {
    return getDisplayModeById(mActiveModeId);
}

sp<disp_mode> QDCM::getDefaultDisplayMode() {
    int32_t id = 0;
...
sp<disp_mode> QDCM::getLocalSRGBMode() {
    char path[PATH_MAX];
...
sp<disp_mode> QDCM::getLocalDCIP3Mode() {
    char path[PATH_MAX];
...
status_t QDCM::getPictureAdjustmentRanges(HSICRanges& ranges) {
    hsic_ranges r;
    memset(&r, 0, sizeof(struct hsic_ranges));

    status_t rc = QDCMController::getInstance().get_global_pa_range(mHandle, 0, &r);
    if (rc == OK) {
...
bool QDCM::hasFeature(Feature feature) {
    ...
    if (QDCMController::getInstance().get_feature_version(mHandle, id, &v, &flags) == 0) {
        if (v.x > 0 || v.y > 0 || v.z > 0) {

            // Color balance depends on calibration data in QDCM
...
status_t QDCM::saveInitialDisplayMode() {
    int32_t id = 0;
    uint32_t flags = 0;
    if (Utils::readInitialModeId(&id) != OK || id < 0) {
        if (QDCMController::getInstance().get_default_display_mode(mHandle, 0, &id, &flags) == OK && id >= 0) {
...
status_t QDCM::getColorBalanceRange(Range& range) {
    status_t rc = QDCMController::getInstance().get_global_color_balance_range(mHandle, 0, &range);
    ALOGV("getColorBalanceRange: min=%d max=%d step=%d", range.min, range.max, range.step);
    return rc;
}

status_t QDCM::getPictureAdjustment(HSIC& hsic) {
    uint32_t enable = 0;
    hsic_config config;
    memset(&config, 0, sizeof(struct hsic_config));

    status_t rc = QDCMController::getInstance().get_global_pa_config(mHandle, 0, &enable, &config);
    if (rc == OK) {
...
status_t QDCM::setDisplayMode(int32_t modeID, bool makeDefault) {
    status_t rc = OK;
    ...
    ALOGD("setDisplayMode: current mode=%d", mActiveModeId);

    if (mActiveModeId >= 0) {
        sp<disp_mode> oldMode = getCurrentDisplayMode();
        ALOGD("setDisplayMode: oldMode=%d flags=%d", oldMode->id, oldMode->privFlags);
        if (oldMode->privFlags == PRIV_MODE_FLAG_SYSFS ||
                mode->privFlags == PRIV_MODE_FLAG_SYSFS) {
            ALOGD("disabling old mode");
    ...
    if (rc == OK) {
        ...
        if (makeDefault) {
            ...
            if (mode->privFlags == PRIV_MODE_FLAG_QDCM) {
                rc = QDCMController::getInstance().set_default_display_mode(mHandle, 0, mode->id, 0);
                if (rc != OK) {
                    ALOGE("failed to save mode! %d", rc);
                    return rc;
                }
        ...
        if (rc != OK) {
            ...
            ALOGD("new default PA: %d %f %f %f %f", tmp.hue, tmp.saturation,
    ...
    ALOGD("setDisplayMode: %d default: %d flags: %d", modeID, makeDefault, mode->privFlags);
    return OK;
}

status_t QDCM::setAdaptiveBacklightEnabled(bool enabled) {
    status_t rc = NO_INIT;
...
bool QDCM::isAdaptiveBacklightEnabled() {
    return mCachedFOSSStatus;
}

int32_t QDCM::getColorBalance() {
    int32_t value = -1;
    uint32_t flags = 0;
    if (QDCMController::getInstance().get_global_color_balance(mHandle, 0, &value, &flags) != 0) {
        value = 0;
    }
    return value;
}

status_t QDCM::setColorBalance(int32_t balance) {
    return QDCMController::getInstance().set_global_color_balance(mHandle, 0, balance, 0);
}

status_t QDCM::setPictureAdjustment(const HSIC& hsic) {
    hsic_config config;
    memset(&config, 0, sizeof(struct hsic_config));
    ...
    return QDCMController::getInstance().set_global_pa_config(mHandle, 0, 1, &config);
}

HSIC QDCM::getDefaultPictureAdjustment() {
