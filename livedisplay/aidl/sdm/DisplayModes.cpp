...
ndk::ScopedAStatus DisplayModes::setDisplayMode(int32_t in_modeID, bool in_makeDefault,
                                                bool* _aidl_return) {
    DisplayMode current_mode = getCurrentDisplayModeInternal();

    if (current_mode.id >= 0 && current_mode.id == in_modeID) {
        *_aidl_return = true;
        return ndk::ScopedAStatus::ok();
    }

    DisplayMode mode = getDisplayModeById(in_modeID);
    if (mode.id < 0) {
        *_aidl_return = false;
        return ndk::ScopedAStatus::ok();
    }

    if (controller_->setActiveDisplayMode(in_modeID) != OK) {
        *_aidl_return = false;
        return ndk::ScopedAStatus::ok();
    }

    if (in_makeDefault && controller_->setDefaultDisplayMode(in_modeID) != OK) {
        *_aidl_return = false;
        return ndk::ScopedAStatus::ok();
    }
