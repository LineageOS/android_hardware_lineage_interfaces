...
#define LOG_TAG "android.hardware.usb.aidl-service-basic"
...
void Usb::queryPortStatusHelper() {
    ...
    portStatus.usbDataStatus = {mUsbDataEnabled ? UsbDataStatus::ENABLED : UsbDataStatus::DISABLED_FORCE};
...
::ndk::ScopedAStatus Usb::enableUsbData(const std::string& in_portName, bool in_enable,
    ...
    std::lock_guard<std::mutex> lock(mMutex);
    if (mCallback != nullptr) {
        auto ret = mCallback->notifyEnableUsbDataStatus(
                in_portName, in_enable, success ? Status::SUCCESS : Status::ERROR,
                in_transactionId);
        if (!ret.isOk()) {
...
::ndk::ScopedAStatus Usb::limitPowerTransfer(const std::string& /*in_portName*/,
                                              bool /*in_limit*/,
                                              int64_t in_transactionId) {
    std::lock_guard<std::mutex> lock(mMutex);
    if (mCallback != nullptr) {
...
::ndk::ScopedAStatus Usb::resetUsbPort(const std::string& /*in_portName*/,
    ...
    std::lock_guard<std::mutex> lock(mMutex);
    if (mCallback != nullptr) {
        auto ret = mCallback->notifyResetUsbPortStatus("", Status::NOT_SUPPORTED,
                                                       in_transactionId);
        if (!ret.isOk()) {
...
::ndk::ScopedAStatus Usb::queryStaticPortInformation(const std::string& /*in_portName*/,
    ...
    if (mCallback != nullptr) {
        ...
        auto ret = mCallback->notifyQueryStaticPortInformation("", staticPortInfo,
