...
// Has to be in the same order as FastChargeMode
constexpr std::array<std::string, 3> fastChargeNodeValues = {
        HEALTH_FAST_CHARGE_VALUE_NONE,
        HEALTH_FAST_CHARGE_VALUE_FAST_CHARGE,
        HEALTH_FAST_CHARGE_VALUE_SUPER_FAST_CHARGE,
};
...
#ifdef HEALTH_FAST_CHARGE_SUPPORTS_FAST_CHARGE
    *_aidl_return |= static_cast<int>(FastChargeMode::FAST_CHARGE);
#endif

#ifdef HEALTH_FAST_CHARGE_SUPPORTS_SUPER_FAST_CHARGE
...
ndk::ScopedAStatus FastCharge::getFastChargeMode(FastChargeMode* _aidl_return) {
    ...
    auto it = std::find(fastChargeNodeValues.cbegin(), fastChargeNodeValues.cend(), content);
    ...
    *_aidl_return =
            static_cast<FastChargeMode>(1 << std::distance(std::begin(fastChargeNodeValues), it));
#else
...
#ifdef HEALTH_FAST_CHARGE_NODE
    if (!android::base::WriteStringToFile(HEALTH_FAST_CHARGE_NODE,
                                          fastChargeNodeValues[static_cast<int>(in_mode)])) {
