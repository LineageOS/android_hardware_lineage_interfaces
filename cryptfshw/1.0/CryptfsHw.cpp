...
static unsigned char* GetTmpPasswd(const char* passwd) {
    int passwd_len = 0;
    unsigned char* tmp_passwd = nullptr;
    if (passwd) {
        tmp_passwd = new unsigned char[MAX_PASSWORD_LEN];
        if (tmp_passwd) {
            secure_memset(tmp_passwd, 0, MAX_PASSWORD_LEN);
            passwd_len = strnlen(passwd, MAX_PASSWORD_LEN);
            memcpy(tmp_passwd, passwd, passwd_len);
        } else {
            LOG_TO(SYSTEM, ERROR) << __func__ << ": Failed to allocate memory for tmp passwd";
        }
    } else {
...
Return<int32_t> CryptfsHw::setKey(const hidl_string& passwd, const hidl_string& enc_mode) {
    ...
    unsigned char* tmp_passwd = GetTmpPasswd(passwd.c_str());
    if (tmp_passwd) {
...
Return<int32_t> CryptfsHw::updateKey(const hidl_string& oldpw, const hidl_string& newpw,
    ...
    unsigned char* tmp_passwd = GetTmpPasswd(newpw.c_str());
    unsigned char* tmp_currentpasswd = GetTmpPasswd(oldpw.c_str());
    if (tmp_passwd) {
