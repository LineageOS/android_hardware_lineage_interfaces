...
Return<int32_t> CryptfsHw::setKey(const hidl_string& passwd, const hidl_string&) {
    ...
    std::memcpy(req.hash32, static_cast<std::string>(passwd).substr(0, QSEECOM_HASH_SIZE).c_str(),
                QSEECOM_HASH_SIZE);
...
Return<int32_t> CryptfsHw::updateKey(const hidl_string& oldpw, const hidl_string& newpw,
    ...
    std::memcpy(req.current_hash32,
                static_cast<std::string>(oldpw).substr(0, QSEECOM_HASH_SIZE).c_str(),
                QSEECOM_HASH_SIZE);
    std::memcpy(req.new_hash32,
                static_cast<std::string>(newpw).substr(0, QSEECOM_HASH_SIZE).c_str(),
                QSEECOM_HASH_SIZE);
