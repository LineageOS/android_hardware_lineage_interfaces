...
Return<int32_t> CryptfsHw::setKey(const hidl_string& passwd, const hidl_string&) {
    ...
    std::strncpy(req.hash32, passwd.c_str(), QSEECOM_HASH_SIZE);
...
Return<int32_t> CryptfsHw::updateKey(const hidl_string& oldpw, const hidl_string& newpw,
    ...
    std::strncpy(req.current_hash32, oldpw.c_str(), QSEECOM_HASH_SIZE);
    std::strncpy(req.new_hash32, newpw.c_str(), QSEECOM_HASH_SIZE);
