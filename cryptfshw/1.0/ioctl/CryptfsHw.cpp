...
Return<int32_t> CryptfsHw::setKey(const hidl_string& passwd, const hidl_string&) {
    ...
    std::memcpy(req.hash32, passwd.c_str(),
                passwd.size() > QSEECOM_HASH_SIZE ? QSEECOM_HASH_SIZE : passwd.size());
...
Return<int32_t> CryptfsHw::updateKey(const hidl_string& oldpw, const hidl_string& newpw,
    ...
    std::memcpy(req.current_hash32, oldpw.c_str(),
                oldpw.size() > QSEECOM_HASH_SIZE ? QSEECOM_HASH_SIZE : oldpw.size());
    std::memcpy(req.new_hash32, newpw.c_str(),
                newpw.size() > QSEECOM_HASH_SIZE ? QSEECOM_HASH_SIZE : newpw.size());
