...
int Controller::createKey(int usage, unsigned char* hash32) {
    ...
    if (!hash32) {
        secure_memset(req.hash32, 0, QSEECOM_HASH_SIZE);
    } else {
        memscpy(req.hash32, QSEECOM_HASH_SIZE, hash32, QSEECOM_HASH_SIZE);
    }

    ret = mFn_create_key(usage, req.hash32);
    if (ret) {
...
int Controller::updateKey(int usage, unsigned char* current_hash32, unsigned char* new_hash32) {
    ...
    if (!current_hash32) {
        secure_memset(req.current_hash32, 0, QSEECOM_HASH_SIZE);
    } else {
        memscpy(req.current_hash32, QSEECOM_HASH_SIZE, current_hash32,
                QSEECOM_HASH_SIZE);
    }
    if (!new_hash32) {
        secure_memset(req.new_hash32, 0, QSEECOM_HASH_SIZE);
    } else {
        memscpy(req.new_hash32, QSEECOM_HASH_SIZE, new_hash32, QSEECOM_HASH_SIZE);
    }

    ret = mFn_update_key_user_info(usage, req.current_hash32, req.new_hash32);
    if (ret) {
