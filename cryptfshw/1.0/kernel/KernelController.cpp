...
int Controller::createKey(int usage, const char* passwd) {
    ...
    GetTmpPasswd(passwd, req.hash32, QSEECOM_HASH_SIZE);
...
int Controller::updateKey(int usage, const char* oldpw, const char* newpw) {
    ...
    req.usage = static_cast<qseecom_key_management_usage_type>(usage);
    GetTmpPasswd(oldpw, req.current_hash32, QSEECOM_HASH_SIZE);
    GetTmpPasswd(newpw, req.new_hash32, QSEECOM_HASH_SIZE);
